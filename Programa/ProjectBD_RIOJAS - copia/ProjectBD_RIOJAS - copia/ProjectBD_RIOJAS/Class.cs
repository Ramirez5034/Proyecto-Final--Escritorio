...
{
    public partial class Class : MaterialSkin.Controls.MaterialForm
    {
        public Class()
        {
            InitializeComponent();
        ...
        public DataTable llenar_grid()
        {
            ...
            string consulta = "SELECT * FROM Class";
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            textBox2.Text = dataGridView1.CurrentRow.Cells["CourseID"].Value.ToString();
            textBox3.Text = dataGridView1.CurrentRow.Cells["PeriodID"].Value.ToString();
        ...
        {
            Conexion.Conectar();
            string insertar = "INSERT INTO Class (EmployeeID,CourseID,PeriodID)VALUES(@EmployeeID,@CourseID,@PeriodID)";
            SqlCommand cmd1 = new SqlCommand(insertar, Conexion.Conectar());
            cmd1.Parameters.AddWithValue("@EmployeeID", textBox1.Text);
            cmd1.Parameters.AddWithValue("@CourseID", textBox2.Text);
            cmd1.Parameters.AddWithValue("@PeriodID", textBox3.Text);
        ...
        {
            Conexion.Conectar();
            string actualizar = "UPDATE Class SET " +
                                 "EmployeeID=@EmployeeID, " +
                                 "CourseID=@CourseID, " +
                                 "PeriodID=@PeriodID " +
                                 "WHERE ClassID=@ClassID"; // Corregí la condición WHERE

            using (SqlCommand cmd2 = new SqlCommand(actualizar, Conexion.Conectar()))
            {
                int classID = ObtenerValorClassIDDesdeFuenteDeDatos();

                cmd2.Parameters.AddWithValue("@ClassID", classID);
                cmd2.Parameters.AddWithValue("@EmployeeID", textBox1.Text);
                cmd2.Parameters.AddWithValue("@CourseID", textBox2.Text);
                cmd2.Parameters.AddWithValue("@PeriodID", textBox3.Text);
        ...
        private int ObtenerValorClassIDDesdeFuenteDeDatos()
        {
            // Puedes ajustar esto según tu lógica y la fuente de datos que estás utilizando
            if (dataGridView1.CurrentRow != null)
            {
                return Convert.ToInt32(dataGridView1.CurrentRow.Cells["ClassID"].Value);
            }
            else
            {
        ...
        {
            Conexion.Conectar();
            string eliminar = "DELETE FROM Class WHERE ClassID = @ClassID";
